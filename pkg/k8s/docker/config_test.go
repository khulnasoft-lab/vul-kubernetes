...
func TestNewBasicAuth(t *testing.T) {
	assert.Equal(t, BasicAuth("Zm9vOmJhcg=="), NewBasicAuth("foo", "bar"))
}
...
func TestGetServerFromDockerAuthKey(t *testing.T) {
	testCases := []struct {
		...
		{
			authKey:        "rg.pl-waw.scw.cloud/vuloperator",
	...
	for _, tc := range testCases {
		t.Run(tc.authKey, func(t *testing.T) {
			server, err := GetServerFromDockerAuthKey(tc.authKey)
...
func TestGetServerFromImageRef(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.imageRef, func(t *testing.T) {
			server, err := GetServerFromImageRef(tc.imageRef)
			require.NoError(t, err)
			assert.Equal(t, tc.expectedServer, server)
		})
	}
...
func TestBasicAuth_Decode(t *testing.T) {
	testCases := []struct {
		...
		{
			name: "Decode GCR",
			v:    BasicAuth(""),
			want: "_json_key",
			want1: `{
